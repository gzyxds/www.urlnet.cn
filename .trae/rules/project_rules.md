...
## 🛠️ 核心技术栈
- **构建工具**: Vite 7 (`vite`, `@vitejs/plugin-react`)
...
### 架构说明
项目采用 **预渲染 (SSG) + 动态元数据** 的SEO方案：
1. **构建时**: `vite build` 生成基础SPA
2. **Sitemap生成**: `scripts/generate-sitemap.js` 扫描路由生成sitemap.xml
3. **预渲染**: `scripts/prerender.js` 使用Puppeteer渲染每个路由为静态HTML

### 使用方式
每个页面组件必须调用 `usePageMetadata` Hook 设置元数据：
```tsx
usePageMetadata({
  title: '页面标题 - 艺创AI',
  description: '页面描述，建议150字符以内',
...
| `dist/sitemap.xml` | 站点地图（构建时生成） |
| `dist/**/*.html` | 预渲染静态页面（构建时生成） |

### 构建命令
```bash
npm run build        # 标准构建 (SPA模式)
npm run build:static # 静态预渲染构建 (需本地浏览器)
```
